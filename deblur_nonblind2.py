...
class PatchSequence(keras.utils.Sequence):
    ...
    def __getitem__(self,n):
        ...
        batch_y= [self.img[y,x,:] for x,y in self._get_pos_range(start_pos,end_pos)]
...
def make_patches(img):
    return PatchSequence(img,BATCH_SIZE)
...
#    last_layer_out=reshaped
    ...
    for c in range(4):
        ...
        last_layer_out=layers.Conv2D(filters,(conv_size,conv_size),strides=(1,1),activation='relu',padding='same')(last_layer_out)
        conv_size//=2
...
model=make_model(gt.shape)
model.summary()


x=tf.expand_dims(blurred,0)
y=tf.expand_dims(gt,0)
print("Made x,y")
...
def fit_thread():
    model.fit(x,y,epochs=1000,steps_per_epoch=None,callbacks=[PlotCallback()])
